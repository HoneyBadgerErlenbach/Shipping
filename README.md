...
- `TAILSCALE_AUTHKEY` — generate one at tailscale.com → Admin → Keys → Auth Keys
...
Set these two values — everything else is auto-generated by the next script:

| Key | Value |
|-----|-------|
| `KIKI_API_KEY` | A fixed shared secret — **the same value for every device**. Copy it from `/home/pi/kiki-api/.env` on the cloud server. Every device sends this key when calling the cloud API (welcome emails, motion alerts, heartbeats). |
| `TAILSCALE_AUTHKEY` | Auth key from tailscale.com/admin — generate a **new one per device** |

Save and close (`Ctrl+X → Y → Enter`).

> The script will **refuse to finish** if `KIKI_API_KEY` is missing or blank.
